#create the nightlife ratings
#run with ./manage.py runscript --traceback utils.pois.score_nightlife
...
def score_nightlife():
    ...
    for hood in raw_scores:
        ...
        h.nightlife_rating = normalized_score
        h.save()

def find_poi_hoods():
    hoods = Neighborhood.objects.all()
    for poi in PointsOfInterest.objects.all():
        for hood in hoods:
            if is_in((poi.latitude, poi.longitude), hood.border):
                poi.neighborhood = hood
                poi.save()
                break
